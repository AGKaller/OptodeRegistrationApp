...
%   [pos, tri]          = refine(pos, tri, _, _, prgrsCB, prgrsMinMax)
%
...
%
% Modified by Konrad Schumacher


setProgrs =  nargin > 4 && ~isempty(varargin{2});
if setProgrs 
    prgrsCB = varargin{2};
end
if ~setProgrs || nargin < 6 || isempty(varargin{3})
    prgrsMinMax = [0 1];
else
    prgrsMinMax = varargin{3};
end
validateattributes(prgrsMinMax,'numeric',{'vector','numel',2,'increasing'},mfilename,'prgrsRange',6);
...
switch lower(method)
  case 'banks'
    npnt   = size(pos,1);
    ntri   = size(tri,1);
    prgrs = (1:ntri)/ntri * prgrsRange + prgrsMinMax(1);
